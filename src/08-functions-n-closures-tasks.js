...
function logger(func, logFunc) {
  return (...param) => {
    ...
    logFunc(`${func.name}(${paramString.substring(1, paramString.length - 1)}) starts`);
    const result = func(...param);
    logFunc(`${func.name}(${paramString.substring(1, paramString.length - 1)}) ends`);
    return result;
  };
}
...
function getIdGeneratorFunction(startFrom) {
  let res;
  return function getValue() {
    if (typeof res === 'undefined') res = startFrom - 1;
    res += 1;
    return res;
  };
}
