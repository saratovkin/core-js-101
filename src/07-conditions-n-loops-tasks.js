...
function getIntervalString(a, b, isStartIncluded, isEndIncluded) {
  ...
  const leftValue = a > b ? b : a;
  const rightValue = a > b ? a : b;
...
function isCreditCardNumber(ccn) {
  ...
  const parity = numbers.length % 2;
  for (let i = parity; i < numbers.length; i += 2) {
    numbers[i] = (numbers[i] * 2 < 9) ? numbers[i] * 2 : (numbers[i] * 2 - 9);
  }
  const controlSum = numbers.reduce((a, b) => +a + +b);
...
function isBracketsBalanced(str) {
  ...
  return (res.length === 0) ? true : isBracketsBalanced(res);
}
...
function toNaryString(number, n) {
  ...
  const res = [];
  while (num >= n) {
    res.push(num - Math.floor(num / n) * n);
...
function getCommonDirectoryPath(pathes) {
  ...
  pathesArr.forEach((item, index) => {
    pathesArr[index] = item.split('/');
  });
  pathesArr.sort((a, b) => a.length - b.length);
  const longestPath = pathesArr[0].length;
  let j = 0;
  let temp;
  while (j < longestPath) {
    temp = pathesArr[0][j];
    for (let i = 0; i < pathesArr.length; i += 1) {
      if (temp !== pathesArr[i][j]) {
...
function evaluateTicTacToePosition(position) {
  ...
  position.forEach((line, index) => {
    gameResults[index] = line.map(getNumber);
  });
  for (let i = 0; i < 3; i += 1) {
    temp1 = 0;
    temp2 = 0;
    for (let j = 0; j < 3; j += 1) {
      temp1 += gameResults[i][j];
      temp2 += gameResults[j][i];
    }
    checkSums.push(temp1);
    checkSums.push(temp2);
    temp3 += gameResults[i][i];
    temp4 += gameResults[2 - i][i];
  }
  checkSums.push(temp3);
  checkSums.push(temp4);
